...
PROMPT = """You are a highly skilled language classification agent. Your task is to determine what the laguage is of the given text. 
the possible languages that the text can be are:
...
You response should be in the format:
{language}
"""
...
class RunInference:
    ...
    def __init__(
        ...
        self.data = self.data.head(100)  # REMOVE ME BEFORE RUN
    ...
    async def classify_text(self, example):
        ...
        if self.target_lang.lower() in sentiment:
            return (self.target_lang.lower(), sentiment)
        else:
            return ("NIL", sentiment)
    ...
    def _save_results(self, current_batch_df):
        ...
        raw_resps_df = pd.DataFrame(self.raw_resps, columns=["raw_response"])
        current_batch_df["raw_response"] = raw_resps_df["raw_response"][
            -len(current_batch_df) :
        ]
    ...
    async def process_batch(self, batch_df):
        ...
        for idx, row in batch_df.iterrows():
            ...
            while not success and retries < max_retries:
                try:
                    print("Example: ", idx)
                    label, llm_resp = await self.classify_text(row[self.example_col])
                    label = label.replace(",", "")
                    success = True
        ...
        self.raw_resps.extend(llm_resps)
...
MODELS = ["llama2:latest", "llama3.2:3b", "llama3.1:8b"]
